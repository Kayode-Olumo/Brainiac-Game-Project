...
let timeCounter;
let secCounter = 60; //sets the timer to start at 60 seconds
...
function cardClick(i) {
	if (moves < maxNoOfMoves && lives > 0) {
		cardClickAction(i);
	} else {
		if(lives > 0) {
			document.getElementById('health-star-'+health).remove();
			cardClickAction(i);
			alert("You've run out of moves! Taking away one life!")
			lives = lives - 1;
			moves = 0;
			health++
		}
